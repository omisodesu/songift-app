    ...
    if (appEnv === "prod") {
      ...
      if (slackWebhookUrl) {
        ...
        if (plan === "nursingHome") {
          ...
          slackMessage = `🎉 *新しい注文が入りました！*\n\n*注文ID:* ${orderId}\n*プラン:* ニコ超2026 8bit\n*お名前:* ${formData.targetName}\n*ジャンル:* ${formData.ncGenre}\n*対象:* ${formData.ncRecipientType}\n*好きなところ:* ${formData.ncFavoritePoint}\n*願い:* ${formData.ncWish}\n*メール:* ${email}`;
        } else {
...
/**
 * ニコ超2026 8bitモード用ジャンル → 音楽要素の変換
 */
const NC_GENRE_TO_MUSIC = {
  "RPG風": {
...
/**
 * ニコ超2026 8bitモード用 願い → 追加タグの変換
 */
const NC_WISH_TO_TAGS = {
 ...
 */
function buildNiconico2026Prompt(order) {
...
【最重要・絶対厳守】lyricsの歌詞はひらがなのみで書く。漢字・カタカナを1文字でも使ってはいけない。
- 例: 「秋の日に生まれて」→「あきのひにうまれて」、「家族の温もり」→「かぞくのぬくもり」、「人と触れ合い」→「ひととふれあい」
- 思いついた歌詞に漢字・カタカナが含まれていたら、必ず読み(よみ)のひらがなに直してから出力する
- 例外はセクションタグ([Chorus]等、英語の[...])のみ。それ以外は呼び名も含め100%ひらがな
...
lyricsは、Q2のジャンルに対応する下記テンプレート(A/B/C)を使い、セクションタグ([Chorus]
[Short instrumental] [Verse] [Pre-Chorus: build up] [Chorus: peak])はそのまま残し、( )の
プレースホルダだけを創作した歌詞で置き換えてください。改行は \\n で表します。
- セクションタグは必ず全て残す(省略禁止)。タグは英語の[...]のまま
- ( )内の歌詞・呼び名は例外なくすべてひらがな(漢字・カタカナ禁止)
- ハッピバースデーの行の呼び名もひらがな(例:マサルさん→まさるさん)

▼ A. 演歌 のときのlyrics:
[Chorus]\\nハッピバースデー (呼び名ひらがな)\\n[Verse]\\n(季節1パターン+Q5から創作・1行目 すべてひらがな)\\n(季節1パターン+Q5から創作・2行目 すべてひらがな)\\n[Pre-Chorus: build up]\\n(Q6から創作・1行目 すべてひらがな)\\n(Q6から創作・2行目 すべてひらがな)\\n[Chorus: peak]\\nハッピバースデー (呼び名ひらがな)\\nハッピバースデー (呼び名ひらがな)\\nハッピバースデー (呼び名ひらがな)

▼ B. 昭和歌謡・フォークソング・ジャズ のときのlyrics:
[Chorus]\\nハッピバースデー (呼び名ひらがな)\\nハッピバースデー (呼び名ひらがな)\\n[Short instrumental]\\n[Verse]\\n(季節1パターン+Q5から創作・1行目 すべてひらがな)\\n(季節1パターン+Q5から創作・2行目 すべてひらがな)\\n[Pre-Chorus: build up]\\n(Q6から創作・1行目 すべてひらがな)\\n(Q6から創作・2行目 すべてひらがな)\\n[Chorus: peak]\\nハッピバースデー (呼び名ひらがな)\\nハッピバースデー (呼び名ひらがな)\\nハッピバースデー (呼び名ひらがな)
...
{
  "lyrics": "(上記A/B/CのうちQ2のジャンルに対応するテンプレートを、セクションタグを残したまま\\nで連結した1行の文字列。歌詞・呼び名はすべてひらがな)",
...
- lyricsには出力形式のセクションタグ([Chorus] [Short instrumental] [Verse] [Pre-Chorus: build up] [Chorus: peak])を必ず全て含める(1つでも省略しない)
- セクションタグ[...]は英語のまま。それ以外の歌詞・呼び名は例外なくすべてひらがな(漢字・カタカナは使わない)
- Q3の呼び名はひらがなに変換して使用(例:マサルさん→まさるさん。カタカナ・漢字はひらがなへ)
...
【出力前の最終チェック】lyricsを1文字ずつ確認し、セクションタグ[...]以外に漢字・カタカナが
1文字でも残っていたら、すべてひらがな(よみ)に直してから出力すること。
  `.trim();
}
...
async function processPromptStep(orderRef, order, orderId) {
  ...
  } else if (order.plan === "niconico2026") {
    systemPrompt = buildNiconico2026Prompt(order);
